...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && ($_POST['accion'] ?? '') === 'crear') {
    ...
    if (!verificarCsrf($_POST['csrf'] ?? null)) {
        ...
        header('Location: pinchazos.php'); exit;
    }

    // ---------- VALIDACIÓN: CAMPOS OBLIGATORIOS ----------
    // (observaciones, ubicación y fotos son OPCIONALES)
    $camposReq = [
        'id_contrato'    => 'ID Contrato',
        'cliente'        => 'Cliente / Empresa',
        'obra'           => 'Obra',
        'tipo_maquina'   => 'Tipo / modelo de máquina',
        ...
        'telefono'       => 'Teléfono',
        'email'          => 'Email',
        'posicion_rueda' => 'Posición de la rueda',
    ];
    $faltantes = [];
    ...
    $modelo     = limpiar($_POST['tipo_maquina']);
    $posicion   = limpiar($_POST['posicion_rueda']);

    if (!idSedePorNombre($sedeNombre)) {
        flash('error', 'La sede seleccionada no es válida.');
        header('Location: pinchazos.php'); exit;
    }
    if (!array_key_exists($posicion, POSICIONES_RUEDA)) {
        flash('error', 'Selecciona la posición de la rueda afectada.');
        header('Location: pinchazos.php'); exit;
    }
    if (!filter_var($_POST['email'] ?? '', FILTER_VALIDATE_EMAIL)) {
        ...
        try {
            ...
            $idIncidencia = crearIncidencia('pinchazo', $datosIncidencia);

            $stmt = $pdo->prepare('
                INSERT INTO incidencias_pinchazo
                    (id_incidencia, posicion_rueda)
                VALUES (?, ?)
            ');
            $stmt->execute([
                $idIncidencia,
            ...
            $pdo->rollBack();
            flash('error', 'Error al guardar el pinchazo: ' . $e->getMessage());
            header('Location: pinchazos.php'); exit;
        }

        $fotos = guardarFotosSubidas('pinchazo', $idIncidencia, $_FILES['fotos'] ?? []);

        $datosEmail = $datosIncidencia + ['posicion_rueda' => $posicion];
        $cuerpo  = construirCuerpoEmail('PINCHAZO', $datosEmail, $fotos);
        $asunto  = 'Reporte de Pinchazo - Sede ' . $sedeNombre . ' (' . $posicion . ')';
        $resMail = enviarEmailSede($sedeNombre, $asunto, $cuerpo);

        marcarIncidenciaEnviada($idIncidencia, $resMail);

        if ($resMail['ok']) {
            flash('success', '✅ Pinchazo comunicado a la sede ' . $sedeNombre
                            . ' (' . $modelo . ' · ' . $posicion . ')');
        } else {
            flash('warning', '⚠️ Pinchazo registrado pero el email a la sede falló. '
                            . 'Causa: ' . ($resMail['error'] ?? 'desconocida'));
        }

        header('Location: pinchazos.php');
        exit;
}
...
if ($u['rol'] !== 'operario') {
    ...
    $filtros = construirFiltrosListado($_GET, ['posicion' => true]);
    $sql = "
        SELECT i.id_incidencia, i.sede_nombre, i.cliente_nombre,
               i.maquina_modelo, i.fecha_creacion,
               p.posicion_rueda,
               usr.username AS creador_username
        FROM   incidencias i
        INNER JOIN incidencias_pinchazo p ON p.id_incidencia = i.id_incidencia
        LEFT JOIN  usuarios usr           ON usr.id_usuario  = i.id_usuario
        WHERE  i.tipo = 'pinchazo' AND {$alcance['where']} {$filtros['where']}
...
renderHeader('Pinchazo', 'fa-screwdriver-wrench');
?>
...
<section class="page-header mb-4">
    <div class="page-icon-badge bg-primary-subtle text-primary">🛞</div>
    <div>
        <h1>Asistencia Neumáticos</h1>
        <p class="text-muted">Parte de incidencia en obra</p>
    </div>
</section>
...
<form method="post" enctype="multipart/form-data" class="form-card mb-5" novalidate>
        ...
        <div class="row g-3">
            ...
            <div class="col-md-6">
                <select class="form-select" name="tipo_maquina" required>
                    <option value="">— Tipo / modelo de máquina —</option>
    ...
    <fieldset class="form-section">
        <legend>Posición de la rueda</legend>
        <div class="row g-2">
            <?php foreach (POSICIONES_RUEDA as $valor => $etiqueta): ?>
                <div class="col-6">
                    <label class="wheel-card w-100">
                        <input type="radio" name="posicion_rueda" value="<?= escapar($valor) ?>" class="d-none" required>
    ...
    <fieldset class="form-section">
        ...
        <div class="d-flex gap-2 mb-2">
            <button type="button" class="btn btn-outline-primary" onclick="obtenerGPS('ubicacionPinchazo')">
                📍 Obtener GPS
            </button>
            <input type="text" class="form-control" id="ubicacionPinchazo" name="ubicacion"
    ...
    <fieldset class="form-section">
        <legend>Observaciones <small class="text-muted" style="font-size:10px;text-transform:none;letter-spacing:0;font-weight:500;">(opcional)</small></legend>
        <textarea class="form-control" name="observaciones" rows="3"
                  placeholder="Detalles técnicos del pinchazo..."></textarea>
    </fieldset>
...
<section class="mt-5">
    <h2 class="section-title">
        ...
        $tituloListado = match($u['rol']) {
            'admin' => 'Últimos pinchazos (todas las sedes)',
            'sede'  => 'Pinchazos de ' . $u['sede'],
            default => 'Mis últimos pinchazos',
        };
        ?>
    ...
    <form method="get" class="bg-white rounded-3 shadow-sm p-3 mb-3 row g-2 align-items-end">
        ...
        <?php endif; ?>
        <div class="col-md-2">
            <label class="form-label" style="font-size:12px;">Posición</label>
            <select name="posicion" class="form-select form-select-sm">
                <option value="">Todas</option>
                <?php foreach (POSICIONES_RUEDA as $val => $etq): ?>
                    <option value="<?= escapar($val) ?>"
                            <?= ($_GET['posicion'] ?? '') === $val ? 'selected' : '' ?>>
                        <?= escapar($val) ?>
                    </option>
                <?php endforeach; ?>
            </select>
        ...
        <?php if ($filtros['activos']): ?>
            <div class="col-12 mt-2">
                <small class="text-muted">
                    ...
                    · <a href="pinchazos.php">Limpiar filtros</a>
                </small>
            </div>
        <?php endif; ?>
    </form>
    ...
    <?php if (empty($listado)): ?>
        <div class="alert alert-light text-center py-4">
            ...
            <?= $filtros['activos'] ? 'Ningún pinchazo coincide con los filtros aplicados.' : 'Aún no hay pinchazos registrados.' ?>
        </div>
    <?php else: ?>
            ...
            <table class="table table-hover align-middle mb-0">
                ...
                <tbody>
                    <?php foreach ($listado as $p): ?>
                        <tr>
                            <td><?= escapar($p['sede_nombre']) ?></td>
                            <td><?= escapar($p['cliente_nombre']) ?></td>
                            <td><small><?= escapar($p['maquina_modelo']) ?></small></td>
                            <td><span class="badge bg-primary-subtle text-primary"><?= escapar($p['posicion_rueda']) ?></span></td>
                            <td><small class="text-muted"><?= date('d/m/Y H:i', strtotime($p['fecha_creacion'])) ?></small></td>
                            <td class="text-end">
                                <a href="incidencia.php?id=<?= (int) $p['id_incidencia'] ?>"
...
document.querySelectorAll('.wheel-card input[type=radio]').forEach(r => {
    r.addEventListener('change', () => {
        document.querySelectorAll('.wheel-card').forEach(w => w.classList.remove('active'));
        if (r.checked) r.closest('.wheel-card').classList.add('active');
...
// ================================================================
// CAMPOS REQUERIDOS — observaciones, ubicación y fotos son OPCIONALES
// ================================================================
const CAMPOS_REQUERIDOS = [
    'id_contrato', 'cliente', 'obra', 'tipo_maquina', 'num_maquina', 'sede',
    'nombre', 'apellidos', 'telefono', 'email'
];
const GRUPOS_RADIO_REQUERIDOS = ['posicion_rueda'];
